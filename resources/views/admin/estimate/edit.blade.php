                        ...
                        <div class="col-12 mb-3">
                            <div class="card card-outline">
                                <div class="card-header admin-cart-header">
                                    <h3 class="card-title">{{ __('all.estimate') }} #{{ $estimate->estimate_number }}</h3>
                                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    ...
                                    <div class="billed-to">
                                        <h6><span id="customer_name">{{ $estimate->customer->display_name }}</span></h6>
                                        <p><span id="address"> {{ $estimate->customer->shipping_address1 }}</span><br>
                                            <span id="phone"></span>{{ $estimate->customer->phone }}<br>
                                        </p>
                                    </div>
                                </div>
                                <div class="col-lg-6 text-right">
                                    ...
                                    <input type="date" class="form-control" placeholder="Estimate Due Date"
                                            name="estimate_due_date" id="estimate_due_date"
                                            value="{{ $estimate->estimate_due_date }}">
                                    <br>
                                </div>
                            </div>
                        </div>
                                                                ...
                                                                <select class="form-control select2"
                                                                    ...
                                                                    @foreach ($taxes as $row)
                                                                        <option data-tax_name="{{ $row->name }}"
                                                                            ...
                                                                            {{ $estimate->tax_percentage == $row->percentage ? 'selected' : '' }}>
                                                                            {{ $row->name }}
                                                                            - [{{ $row->percentage }}]
                                                                        </option>
                                                                    @endforeach
            ...
            if (estimateId != "") {
                ...
                $.get(url, function(data) {
                    $.each(data.data, function(key) {
                        ...
                        var product_total = parseInt(data.data[count].product_quantity) * parseFloat(
                            data.data[count].product_price);
                        payable = payable + product_total;
                        var markup = '<tr id="row' + i + '" class="no"> <td class="text-left"> <h6>' +
                            product_name + '<input type="hidden" name="name[]" value="' + product_name +
    ...
    <script>
        (function($) {
            ...
            $(document).on("click", "#submit_estimate", function(event) {
                ...
                $(this).hide();
                var estimate_id = $('#estimateId').val();
                $.ajax({
                    url: "{{ url('admin/estimates/update') }}" + '/' + estimate_id,
                    method: "POST",
                    data: $('#product-form').serialize() + "&date=" + $('#estimate_date').val() + "&due_date=" + $('#estimate_due_date').val() ,
