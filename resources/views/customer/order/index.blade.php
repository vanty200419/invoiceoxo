                    ...
                    <div class="card card-outline card-new-color1">
                        <div class="card-header">
                            ...
                            <a href="{{ url('customer/orders/create') }}" class="btn btn-primary float-right"><i
                                    class="fa fa-plus"></i> &nbsp; {{ __('all.add_new_order') }}</a>
                        </div>
                        <div class="card-body">
                            <div class="e-table">
                                        ...
                                        @if (count($orders) > 0)
                                            @foreach ($orders as $row)
                                                <tr>
                                                        ...
                                                        <div class="d-flex">
                                                            <div class="mt-1">
                                                                <a class="admin-text-black"
                                                                   href="{{ url('customer/orders/show/' . $row->id) }}">{{ $row->order_number }}</a>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            <a href="{{ url('customer/orders/show/' . $row->id) }}"
                                                               class="btn btn-sm admin-submit-btn-grad text-white"><i
                                                                    class="fas fa-eye"></i></a>
                                                        @if ($row->order_status == 0)
                                                            <!--edit and delete restricted on pending state-->
                                                                <a href="{{ url('customer/orders/update/' . $row->id) }}"
                                                                   class="btn btn-sm admin-submit-btn-grad text-white"><i
